...
from .derpn_proposals_layer import DeRPNProposalsLayer
from .generate_derpn_labels_targets_layer import GenerateDeRPNLabelsTargetsLayer
...
class _deRPN(nn.Module):
    ...
    def __init__(self, din):
        ...
        self.deRPN_proposal = DeRPNProposalsLayer(self.feat_stride)
        # 根据anchor回归值微调anchor的大小和位置，获得真正的proposals

        # define anchor target layer
        # self.RPN_anchor_target = _AnchorTargetLayer(self.feat_stride, self.anchor_scales, self.anchor_ratios)
        self.deRPN_anchor_target = GenerateDeRPNLabelsTargetsLayer(self.feat_stride)
    ...
    def forward(self, base_feat, im_info, gt_boxes, num_boxes):
        ...
        rois = self.deRPN_proposal((rpn_cls_prob.data, rpn_bbox_pred.data,
                                  im_info, cfg_key))
        ...
        if self.training:
            ...
            rpn_data = self.deRPN_anchor_target((rpn_cls_score.data, gt_boxes, im_info, rpn_bbox_pred.data))
            ...
            rpn_cls_score = torch.index_select(rpn_cls_score.view(-1, 2), 0,
                                               rpn_keep)  # 在这里对应label中为-1值的位置排除掉score中的值，并且变成[-1,2]的形状方便计算交叉熵loss
            rpn_label = torch.index_select(rpn_label.view(-1), 0,
                                           rpn_keep.data)  # 在这里留下label中的非-1的值，表示对应的anchor与gt的IoU在0.7以上
