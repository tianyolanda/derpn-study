...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        all_anchors = all_anchors.view(K * A, 4)  # 生成全部anchor (h*w*9,4)

        total_anchors = int(K * A)  # total_anchors记录anchor的数目

        keep = ((all_anchors[:, 0] >= -self._allowed_border) &  # xmin >=0
                (all_anchors[:, 1] >= -self._allowed_border) &  # ymin >=0
                (all_anchors[:, 2] < long(im_info[0][1]) + self._allowed_border) &  # xmax <= width
                (all_anchors[:, 3] < long(im_info[0][0]) + self._allowed_border))  #  ymax <= height

        inds_inside = torch.nonzero(keep).view(-1)  # 把刚才筛选掉的出边框的anchor都去掉
...
#           num_bg = cfg.TRAIN.RPN_BATCHSIZE - sum_fg[i]
        ...
        labels = labels.view(batch_size, 1, A * height, width)  # A = 9
        outputs.append(labels)

        bbox_targets = bbox_targets.view(batch_size, height, width, A*4).permute(0,3,1,2).contiguous()
        outputs.append(bbox_targets)  # rpn_bbox_targets
