...
class _RPN(nn.Module):
    ...
    def __init__(self, din):
        ...
        # 首先进行两层的输出维度计算
        # the classiﬁcation layer predicts 2×2N scores to estimate whether
        # an anchor string is matched or not matched with object edges
        ...
        # 【也就是说，cls和reg层在训练的时候，就自己开始训练了，根本不需要先生成anchor，因为anchor是定义好的已经存在的】
        # 前面我们知道，60×40的map上每个位置对应输入图像上的9个anchors，这9个anchors都是固定大小和尺寸的！
        # 我们知道这个就够了，因为整个rpn就是要训练来预测每个位置这些anchors的前背景分类得分和边框回归值。
        # 所以只需要在proposal_layer开始的时候根据经验生成这些anchors，最后在rpn末端对这些anchors进行调整、排序和筛除。
        # 【只是用来训练cls和reg层】

        # DeRPN新增代码分割线---------------------------------------------------------------------------------------------#
        # DeRPN reg层：w，h分别的regression 计算
        # Besides, each anchor string predicts a width segment or height segment with
        # the coordinates of (x, w) or (y, h), respectively.
        ...
        # define anchor target layer 训练才有的
    ...
    def forward(self, base_feat, im_info, gt_boxes, num_boxes):
        ...
        # 经过3*3*512 卷积核：rep_conv1维度 [1,512,height,width]
        ...
        # 最终获得的cls层tensor # [1, 18, height, width]
        ...
        derpn_cls_prob_w = self.reshape(derpn_cls_prob_w_reshape, self.DeRPN_nc_score_out_w)
        # w -- cls层tensor [1,14,height,width]
